...
export function MDAClaimChart({ className = "" }: MDAClaimChartProps) {
  ...
  const processMDAData = useMemo(() => {
    ...
    const formatMonthYear = (year: any, month: any): string | null => {
      ...
      try {
        ...
        const monthAbbrev =
          monthAbbrevMap[monthStr] || monthStr.substring(0, 3);
    ...
    const monthlyData = new Map<
      string,
      { eligible: number; received: number }
    >();
    let processedRows = 0;
    let validRows = 0;
    ...
    allMDAData.forEach((row, index) => {
      ...
      if (index < 10) {
        console.log(
          `MDA Chart Row ${
            index + 1
          }: Year=${year}, Month=${month} -> ${monthYear}, Eligible=${eligibleRaw} -> ${eligible}, Received=${receivedRaw} -> ${received}`
        );
      }
    ...
    console.log(
      `MDA Chart: Processed ${processedRows} rows, ${validRows} valid rows, ${monthlyData.size} monthly data points`
    );
    ...
    return {
      ...
      hasData:
        ...
        (eligibleData.some((v) => v > 0) || receivedData.some((v) => v > 0)),
    };
  ...
  const chartOptions: ApexOptions = {
    ...
    // Fixed fill configuration for area charts
    fill: {
      type: chartType === "area" ? "gradient" : "solid",
      colors: processMDAData.series.map((s) => s.color), // Use individual series colors
      gradient:
        chartType === "area"
          ? {
              shadeIntensity: 1,
